...
begin
  ...
  result = connection.exec("select weight, give_for ,quality ,length from crops;")
  # 取り出した各行を処理する
  result.each do |record|
      # 各行を取り出し、putsでターミナル上に出力する
      puts "ゴーヤの大きさ：#{record["weight"]}　売った相手：#{record["give_for"]} 品質 : #{record["quality"]} 長さ : #{record["length"]}"
  end
ensure
